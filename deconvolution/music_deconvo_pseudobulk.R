...
## pseudobulk data
sce_data = list(sacc = sce.sacc, amyg = sce.amyg)

pb <- map(sce_data,function(sce){
  sce$pb <- paste0(sce$donor)
...
exp <- list(sacc = list(bulk = pb$sacc,
                        sce = sce.sacc), 
            amyg = list(bulk = pb$amyg,
                        sce = sce.amyg)
)
...
# bulk  sce 
# 540 6582 
exp_set <- map(exp, ~list(bulk = ExpressionSet(assayData = .x$bulk),
...
est_prop_long <- map(est_prop, ~melt(.x$Est.prop.weighted) %>%
                       rename(Donor = Var1, cell_type = Var2, prop = value) %>% 
                       arrange(cell_type) )
...
output_prop <- map2(est_prop_long, input_prop, function(est,input){
  est %>% select(donor = Donor, cell_type, prop) %>%
    inner_join(input)
})
...
in_out_prop <- ggplot(output_prop_df, aes(input_prop, prop, color = cell_type, shape = donor))+
  ...
  geom_abline()+
  facet_wrap(~dataset) +
  labs(y = "MuSiC estimated prop - pseudobulk data", x = "Input sn data prop")

ggsave(in_out_prop, filename = "plots/prop_input_vs_ouput_pseudobulk.png")
