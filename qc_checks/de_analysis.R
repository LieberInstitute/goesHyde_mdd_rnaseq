...
### load data
load("count_data/wong_MAC21_rse_gene_annotated_n40.Rdata")
...
boxplot(m_mitoRate ~ Label, data = colData(rse_gene), 
	xlab="",outline = FALSE, ylim = range(rse_gene$m_mitoRate),
	ylab = "chrM mapping rate", las=3)
text(jitter(as.numeric(rse_gene$Label),amount=0.15),
	rse_gene$m_mitoRate,letters[as.numeric(factor(rse_gene$mouse_id))], 
	col=as.numeric(rse_gene$Tissue),font=2)
abline(h=0.5, lty=2)
boxplot(concordMapRate ~ Label, data = colData(rse_gene), 
	xlab="",outline = FALSE, ylim = range(rse_gene$concordMapRate),
	ylab = "concordant mapping rate", las=3)
text(jitter(as.numeric(rse_gene$Label),amount=0.15),
	rse_gene$concordMapRate,letters[as.numeric(factor(rse_gene$mouse_id))], 
	col=as.numeric(rse_gene$Tissue),font=2)
abline(h=0.86, lty=2)
dev.off()
...
## drop one sample
keepIndex=rse_gene_filter$concordMapRate > 0.86
...
palette(brewer.pal(4,"Set1"))
plot(pca$x, pch=as.numeric(rse_gene_filter$Group) + 20, 
	bg=rse_gene_filter$Tissue,cex=1.5,
	xlab = pca_vars_lab[1], ylab = pca_vars_lab[2])
legend("topright", levels(rse_gene_filter$Tissue), 
...
for(i in 1:10) {
	...
	text(jitter(as.numeric(rse_gene_filter$Label),amount=0.15),pca$x[,i],
		letters[as.numeric(factor(rse_gene_filter$mouse_id))], 
		col=as.numeric(rse_gene_filter$Tissue),font=2)
}
dev.off()

### variables
rse_gene_filter$mouse_id = factor(rse_gene_filter$mouse_id)

vars = c("rna_conc_ug_u_l", "x260_280", "total_rna_given_ug",
	"overallMapRate", "concordMapRate", 
	"m_mitoRate", "totalAssignedGene",
...
## add anova metrics
regVars = c("Tissue", "Group","mouse_id")
names(regVars) = c("Tissue", "MAC21", "Mouse")
...
## split by tissue
tIndexes = splitit(rse_gene_filter$Tissue)
...
statList_human_chr21 = lapply(tIndexes, function(ii) {
	...
	## mean-variance
	mod = model.matrix(~Group + totalAssignedGene + m_mitoRate,
...
mod = model.matrix(~Group*Tissue + totalAssignedGene + m_mitoRate,
