...
load_rse <- function(feat, reg) {
    ...
    # expmnt data
    load(Sys.glob(here("twas", "filter_data", paste0(opt$region, "_rda"), "MDD_*_hg38_rseGene_rawCounts_allSamples_n*.Rdata")), verbose = TRUE)
    ## Could be more complicated later on for exon, jxn, tx
    rse <- rse_gene
    assays(rse)$raw_expr <- assays(rse_gene)$RPKM

    load(here("twas", "filter_data", paste0(opt$region, "_rda"), "genePCs.Rdata"), verbose = TRUE)
    pcs <- genePCs
