...
if (FALSE) {
    opt <- list('region' = 'sACC', "feature" = "gene", "cores" = 1, "test" = TRUE)
}
...
load_rse <- function(feat, reg) {
    ...
    # expmnt data
    load(Sys.glob(here("twas_both", "filter_data", paste0(opt$region, "_rda"), "MDD_*_hg38_rseGene_rawCounts_allSamples_n*.Rdata")), verbose = TRUE)
    ## Could be more complicated later on for exon, jxn, tx
    rse <- rse_gene
    assays(rse)$raw_expr <- assays(rse_gene)$RPKM

    load(here("twas_both", "filter_data", paste0(opt$region, "_rda"), "genePCs.Rdata"), verbose = TRUE)
    pcs <- genePCs
...
bim_file <-
    here::here(
        ...
        paste0("LIBD_merged_h650_1M_Omni5M_Onmi2pt5_Macrogen_QuadsPlus_dropBrains_maf01_hwe6_geno10_hg38_filtered_", opt$region, "_MDD_uniqueSNPs")
    )
