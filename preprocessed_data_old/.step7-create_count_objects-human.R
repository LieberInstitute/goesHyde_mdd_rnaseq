...
opt <- list('organism' = 'hg38',
	'maindir' = '/dcl01/lieber/ajaffe/lab/goesHyde_mdd_rnaseq/preprocessed_data_old',
	'experiment' = 'goesHyde',
	'prefix' = 'MDD',
	'paired' = TRUE,
	'stranded' = 'reverse',
...
pd = read.csv("samples_phenodata.csv", row.names=1, stringsAsFactors = FALSE)
stopifnot(identical(pd$RNum, metrics$SAMPLE_ID))
metrics = cbind(metrics, pd)


## drop two bad flowcells for now
dropInd = which(metrics$HiseqFC %in% c("HW7WWBBXX","HWC3VBBXX"))
metrics = metrics[-dropInd,]
manifest = manifest[-dropInd,]
N <- length(metrics$SAMPLE_ID)
